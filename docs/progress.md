...
- **2026-08-15 · L7 together（同看）**：`qi/action/together.py`；explore/share/open 池约 12 分钟粘性；她可软邀 + 你可主动 + 见闻卡「一起看」；确认不叠卡；同伴文案；`tests/test_together.py` 绿。真源 `docs/specs/tasks/2026-08-15-L7-together-同看-任务包.md`。删白名单 / irreversible 仍后置。
- **2026-08-15 · L7 write（写下）**：`qi/action/write.py`；D: 白名单（目录+文件）；日记按 `日记-YYYY-MM-DD.md` 新建；确认不叠卡；白话意图；`tests/test_write.py` 绿。真源 `docs/specs/tasks/2026-08-15-L7-write-写下-任务包.md`。删白名单 / irreversible 仍后置。

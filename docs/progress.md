...
测试规模约 **190**（`python -m pytest`，2026-08-01 实测）；层文档实现规格以代码为准。2026-07-25 选择性回写覆盖 L1–L7；**2026-07-26** 再回写 Now/Next/F1/F2（含 L2-user-facts / progress）。v3 Later 有痛点再开，不当施工队列。**2026-08-01** 文档大整顿：确立《栖·数字生命架构方案》为唯一架构方案（后续演进以其 §五 阶段零~四为准）；docs/README.md 立文档宪法；dev/ 清理至 6 份在用手册；thoughts/ 纲要并入卷一，各卷加定位回写。

...
| 键 | 默认值 | 语义 |
|----|--------|------|
| `llm.default_provider` | 见 yaml（现行 `zhipu`） | 默认供应商（OpenAI 兼容；现行走 custom_providers） |
| `llm.providers.*.base_url` / `api_key` | — | 端点与密钥（密钥走 `.env`）；备用见 `custom_providers` |
| `llm.providers.*.models.fast` / `strong` | 见 yaml | 快/强模型名（现行 `glm-5.3-flash`） |
| `llm.custom_providers` | 见 yaml（仅 `zhipu`） | 自定义 OpenAI 兼容端点；非空对象 |
| `llm.model_routing.*` | 见 yaml（现行多为 `zhipu:fast`） | 各意图路由到 provider:model；密钥用 `ZHIPU_API_KEY` |

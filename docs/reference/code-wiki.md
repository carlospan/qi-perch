...
> 本文档由代码自下而上梳理而成，权威以代码为准（**代码 > layers > 现行心智导读**；`explanation/archive/` 为史料）。
> 目的：给新接手者一张"从入口到心跳"的完整地图（非 L1–L7 逐层规格）。
> - **与 `reference/layers/`**：layers 是层实现规格；本文是全景地图。冲突时以**代码**为准，其次 layers。
> - **短导读**：[`docs/explanation/栖·现行心智导读.md`](../explanation/栖·现行心智导读.md)。
> - 与 `docs/explanation/栖·数字生命架构方案.md`（目标架构）配套：本文讲"现在是什么"，那份讲"将来要变成什么"。

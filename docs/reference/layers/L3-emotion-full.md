<!-- 现行路径：reference/layers/L3-emotion-full.md（原 layers/L3-emotion-full.md，2026-08-02 重构迁移；正文以代码为准，未改） -->

# L3 · 情绪完善

> 让栖的情绪从"简单反应"变成"内在天气"。有惯性、有周期、有耦合、有说不清为什么的低落。
>
> <!-- 演进指向(2026-08-01)：情绪动力学是「真骨骼」，整体保留并升级为架构方案 N0 内稳态层；远期接入资源账本（compute/token 消耗→energy 压力，阶段四）。见 docs/explanation/栖·数字生命架构方案.md §四 N0。 -->
...
- `docs/explanation/archive/栖·意识设计.md` → §四（情绪动力学：全部）
- `docs/explanation/archive/栖·意识设计.md` → §九（节奏：日内节律、心跳频率、模式切换）
- `docs/explanation/archive/栖·工程手记.md` → §四（情绪动力学实现代码）
- `docs/reference/contract.md` → "情绪表达"硬规则
...
# <!-- 回写(2026-07-26)：COUPLING_STAGE_SCALE；依据：emotion.py -->
...
def apply_coupling(
    ...
    """deviation * weight * 0.1 * scale；None=全耦合(1.0)；attachment_unmet = 1 - attachment。"""
    scale = 1.0

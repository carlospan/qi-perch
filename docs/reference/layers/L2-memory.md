<!-- 现行路径：reference/layers/L2-memory.md（原 layers/L2-memory.md，2026-08-02 重构迁移；正文以代码为准，未改） -->

# L2 · 记忆

> 让栖记得你。不是"查询数据库"，是"记得那天你跟我说……"
>
> <!-- 演进指向(2026-08-01)：阶段零将做 bge 语义检索（替换 CharNgramEmbeddingFunction）；后续 N4(a) 三修复——episodes+role_map、梦=巩固（积压驱动）、waking 去重。见 docs/explanation/栖·数字生命架构方案.md §五/§四 N4。 -->
...
栖的记忆有四种：**工作记忆**（working，最近对话）、**叙事记忆**（narrative，会褪色的故事）、**身体记忆**（body，交互模式）、**用户事实**（fact，关于你的稳定事实，如名字/身份/家人）。第四种已落地（`qi/memory/facts.py` + Brain 接线），详见 `docs/reference/layers/L2-memory-user-facts.md`。
...
- `docs/explanation/archive/栖·意识设计.md` → §五（记忆：工作记忆、叙事记忆、褪色机制）
- `docs/explanation/archive/栖·工程手记.md` → §三（narrative_memories 表、body_memory 表）
- `docs/reference/contract.md` → "记忆引用"硬规则

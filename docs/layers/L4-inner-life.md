...
>
> <!-- 演进指向(2026-08-01，必读)：本层机制的「体验形态」仍然有效，但「产生方式」（概率触发 + 远程 LLM prompt 生成独白/梦/反思）已被判为表演层——阶段一起替换为内部状态驱动的模板生成，长期按架构方案 N2（GWS+世界模型）/N3（内在动机）重写。请勿按本文件现行规格继续打磨概率触发的 LLM 生成。见 docs/design/栖·数字生命架构方案.md §2.2/§五。 -->

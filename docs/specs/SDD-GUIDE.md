...
### 2.1 固定分工：CodeBuddy 出方案，Cursor 执行编码

本仓库的开发任务**默认**由双 Agent 协作完成，分工固定不可混淆：

- **CodeBuddy（方案侧）：** 仅负责起草任务包、PR 方案、交叉检验协调与最终实施验收；**不写 `qi/` 实现代码**，只产出方案文档（含精确改动点、代码片段骨架、纪律红线、验收清单）。
- **Cursor（执行侧）：** 仅负责按 PR 方案落地编码、自测、回执；不重新设计架构方向，方案有出入时以不破坏现有行为为前提自行适配并说明偏离。

> **为什么固定这样分（减少双方负担）：** 一份方案同时由同一个 Agent 起草又执行，容易把“写文档”和“写代码”混为一谈（阶段三曾出现 CodeBuddy 误把“起草 PR”写成直接敲实现代码的失误）。分工后双方各司其职——CodeBuddy 不必负担编码上下文与大段实现调试，Cursor 不必负担方案权衡与跨包协调，**能显著减少双方负担与失误面**。
...
- 一份任务包配套文件的命名惯例：`YYYY-MM-DD-任务名.md`（任务包本体）、`YYYY-MM-DD-任务名-PR方案.md`（CodeBuddy 出）、`YYYY-MM-DD-任务名-Cursor编码请求.md` / `...-Cursor编码回执.md`（Cursor 侧）。

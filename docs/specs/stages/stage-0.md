...
| 2 | P0 brain 拆分（`brain.py` → `brain_*.py`，**收官时 632 行**，零行为变更，测试全绿） | ✅ |

> **行数注（2026-08-08 审计回写；2026-08-09 再刷）**：判据 #2 的「632」是 P0 收官快照。阶段一～四功能回填后，`brain.py` 编排面约 **1377** 行（仍薄委托 `brain_*.py`；中间曾记 ≈1223）；勿把现行行数当成阶段零未达标。

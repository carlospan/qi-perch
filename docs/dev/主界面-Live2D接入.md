...
> 本文档**取代** `主界面设计-黄昏的枝.md` 中关于形象的部分（§三"静"里的 CSS 光团头像、§六的 `AvatarView.vue`）。
> 其余部分——三视图（静/谈/忆）、设计令牌、情绪→**氛围**映射（天色/光晕/光尘/雾）、动效、红线——**仍然有效**，并叠加在 Live2D 形象之上。
> 形象现在是一个 Live2D 模型，情绪既改变房间的天气（氛围），也改变她的脸色（Live2D 参数）。
...
```
你是栖的前端工程师。先读 docs/contract.md、栖·灵魂书.md、
docs/dev/主界面设计-黄昏的枝.md（氛围/三视图/令牌仍有效），
再读 docs/dev/主界面-Live2D接入.md（本次唯一实现依据，形象部分以它为准）。

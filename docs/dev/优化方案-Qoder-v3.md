...
> **状态：施工蓝图已落地（非待办工单）。**  
> - **已完成：** Q1–Q2、N1–N4、Q3、X1、X2、X4、P4（`6a6056d` + `71ef224`）  
> - **追加已完成：** F1 同拍回声（`ec18ed3`）；F2+N3b 冷启动误判与内在体验禁身体（`2ed96f6`）  

> - **未做 / 仍有效：** §五 Later、§六 Don't——有痛点再开，不要当未勾选的施工项  
> 正文保留「文件/做法/验收」作考古与对照；**现行代码以仓库为准，勿按文中旧行号照抄开工。**  
>  
> **撰写者：** Qoder  
> **日期：** 2026-07-26（v3，详细施工蓝图重写）  
> **基线：** `71bab53`（施工前代码快照）  
> **前置：** `系统质量评估-Qoder-v3.md`（工程尺 8.1 / 存在尺 8.8，同基线快照）  
> **原分工（已完成）：** Qoder 写蓝图；Cursor 执行  
...
Now（养声音与边界）：
  ...
  N2  籍贯/地点事实落库（含测试）
  N4  「忆」实时推送（含第一次）
  F1  同拍回声修复               ← 真实对话实证；已落地 `ec18ed3`
  F2+N3b 冷启动误判 / 内在体验禁身体 ← 已落地 `2ed96f6`

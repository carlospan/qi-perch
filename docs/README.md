...
## ★ 人机协作铁律（2026-08-15 立 · 必读）

> **本仓库怎么和 AI 一起定需求、再写代码——这不是闲聊习惯，是正式工作流。**

| 名字 | 意思 |
|------|------|
| **HITL（人在回路）** | 关键产品决策由**维护者点头**；Agent 不擅自定死 |
| **Spec-driven（规范驱动）** | 先把可观测行为写进任务包 Spec，再编码 |
| **渐进对齐（一块一拍）** | **一次只确认一小块**；每块附 Agent **建议与理由**；谈定立刻写入任务包 / HITL 表，不攒到最后、不靠聊天记录当真源 |
...
> 过程定稿（如体系重构方案）**不**放根目录，进 `specs/archive/`。
...
| 行动意图是否「懂意思」 | `reference/contract.md`「理解意图」 | 口令式触发 = bug；见文档宪法 ★ 铁律第四条 |
...
- **人机协作铁律（HITL + Spec-driven + 渐进对齐）**：见本文开篇 ★；细则 `specs/SDD-GUIDE.md` **§2.5**
- 写 spec / 防 drift 规范：`specs/SDD-GUIDE.md`（**Cursor 单线默认**：短规格 → **§2.5 一块一拍填 HITL** → 编码 → 自验 → 回写）
